...
// Detect iOS - using a more compatible approach
const isIOS = () => {
...
const simulateDownloadProgress = (setDownloadProgress: React.Dispatch<React.SetStateAction<number>>) => {
...
const downloadRecording = async () => {
  ...
  try {
    ...
    // More reliable iOS detection - fixed to avoid TypeScript error
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && 
                  (/WebKit/.test(navigator.userAgent) || /AppleWebKit/.test(navigator.userAgent));
    
    // Force standard download behavior if needed (for debugging)
    const forceStandardDownload = false; // Set to true to test standard download on iOS
    
    if (isIOS && !forceStandardDownload) {
      console.log("iOS device detected, using alternative download method");
      
      // For iOS, use the Web Share API if available
      if (navigator.share && "files" in navigator.share) {
        try {
          console.log("Using Web Share API");
          // Create a File object from the Blob
          const file = new File([audioBlob], `${meetingName}.webm`, { 
            type: audioBlob.type 
          });
          
          // Trigger the native iOS share sheet
          await navigator.share({
            title: `${meetingName} Recording`,
            files: [file]
          });
          ...
          console.log("Falling back to audio preview method");
          saveAsBase64ForIOS();
        }
      } else {
        console.log("Web Share API not available, using audio preview method");
...
// Helper method for iOS to save as base64 and provide download option
const saveAsBase64ForIOS = () => {
  if (!audioBlob) return; // Add null check
  
  // Create a temporary storage for the blob data in localStorage
  ...
  reader.onload = () => {
    try {
      ...
      // Update UI to show "file saved" and provide playback option
      toast({ 
        title: "Recording ready", 
        description: "Recording is available for playback and download below" 
      });
      ...
      // Add a playback section to the page
      const playbackSection = document.createElement('div');
      playbackSection.className = 'mt-4 p-4 border rounded-lg bg-gray-50';
      playbackSection.innerHTML = `
        <h3 class="font-medium text-gray-900 mb-2">Recording Preview</h3>
        <p class="text-sm text-gray-500 mb-3">Listen to your recording:</p>
      `;
      playbackSection.appendChild(audioElement);
      playbackSection.appendChild(document.createElement('br'));
      playbackSection.appendChild(downloadLink);
      ...
      if (previewContainer) {
        ...
        previewContainer.appendChild(playbackSection);
      }
        ...
        <div className="space-y-4">
          ...
          {!downloadClicked && (
            <p className="text-sm text-muted-foreground text-center">
              Please download your recording first
            </p>
          )}
        </div>
      )}
...
function setIsDownloading(value: boolean) {
  // This function is likely a setter for a state variable.
  // Assuming it's meant to update the `isDownloading` state, 
  // we can use the `setIsDownloading` state updater function already defined in the component.
